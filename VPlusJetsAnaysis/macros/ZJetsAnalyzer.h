#ifndef ZJetsAnalyzer_h
#define ZJetsAnalyzer_h
...
//  	TBranch      *b_QGVars;
  	...
  	TBranch      *b_PUWeightSysDown;  	
		...
		ZJetsAnalyzer(TTree *tree = 0, Int_t isample = -1);
		virtual ~ZJetsAnalyzer();
...
#ifdef ZJetsAnalyzer_cxx
ZJetsAnalyzer::ZJetsAnalyzer(TTree *tree, Int_t isample) {
  ...
  file_address_s = "";
	...
	isMC = false;
	if (mysample == 0 || mysample == 91) isMC = true;
	
  if (tree == 0) {

		bool outrange;
		outrange = mysample < 0 || (mysample > 0 && mysample < 91) || (mysample > 91);
		...
		cout << "-----------------------------------------------------" << endl;
		cout << "mysample == 91  -> DYJetsToLL_M-50" << endl;
		cout << "-----------------------------------------------------" << endl;
...
Int_t ZJetsAnalyzer::GetEntry(Long64_t entry) {
...
Long64_t ZJetsAnalyzer::LoadTree(Long64_t entry) {
...
void ZJetsAnalyzer::Init(TTree *tree) {
...
void ZJetsAnalyzer::Show(Long64_t entry) {
...
Int_t ZJetsAnalyzer::Cut(Long64_t entry) {
...
/*
TString ZJetsAnalyzer::floatToString(float number){
	ostringstream oss;
	oss << number;
	return oss.str();
}
*/
#endif // #ifdef ZJetsAnalyzer_cxx
